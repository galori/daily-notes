...
RSpec.describe DailyNotes do
  before do
    @daily_notes_file_test_path = "tmp/2023/07/7.02/daily_notes.md"
  ...
  it 'adds the right contents to the daily notes file' do
    ...
    expected_contents = <<~EOS
      ---
      Title: Daily Notes for July 2nd, 2023
      Date: 07.02.2023
      ---

      # Daily Notes for July 2nd, 2023
  ...
  describe 'opening the IDE after creating the file' do
    ...
    let(:call_string) { "idea #{@base_path} --line 10 #{@base_path}/2023/07/7.02/daily_notes.md &> /dev/null" }
